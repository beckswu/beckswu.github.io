...
- <span style="background-color:#FFFF00">allow parallel computing</span>: allow model running on multiple machines or courses simultaneously
...
- Each box represent the computation made at each steps and colors represent the information used for every computation. 
- Every word in the sequence is multiplied by the same weight $$W_x$$, the informatin propagate from the beginning to the end is multiplied by $$W_h$$. 
...
3. a prediction $$\hat y$$ is computed using current hidden state
...
- GRUs compute significantly more operations, can cause longer processing times and memory usage. <span style="background-color:#FFFF00">GRUS help preserve important information for longer sequences</span>. 
...
- Prone to vanishing or exploding gradients: This can arise <span style="color:red">due to the fact that RNN progagates information from the beginning of the sequence to the end</span>. 
  ...
  - <span style="color:red">Notice: the information from first step doesn't have much influence on the output.</span>. Can see Orange portion from the first step decrease with each new step. Because computation at the first steps don't have much influence on the cost function. 
...
- The series of gates allows gradients to flow unchanged. -> so <span style="background-color:#FFFF00">vanishing/exploding gradient is mitigated</span>
...
3. apdding: LSTM require all sequences <span style="background-color:#FFFF00">are the same length</span>. Can set length of sequnces to a certain number and add generic ```<PAD>``` token to fill all empty spaces.
...
<span style="background-color:#FFFF00">It's made up two identical neural networks which are merged at the end.  Used as Indenify similarity between things</span>

e.g. Comparing meaning not the same as compare words

- How old are you = what is your age
- Where are you from $$ /neq $$ where are you going. Although first three words are the same
...
loss function allow whether model do well. When difference bigger or smaller along x-axis, loss bigger or smaller along the y-axis. Minimizing loss in training-> minimize this difference
...
vector v1, #row = batch sizes,  #columns is the number of embedding layer(```d_model```). Note dimension of embedding ```d_model``` is a parameter that determines the dimensions of weights through each layer in the model, determine the size of output layer. $$v_{1,1}$$ is a duplicate of $$v_{2,1}$$, but $$v_{1,1,}$$ is not a duplicates of any other row in $$v_{1}$$
