...
- function template argument conversion
    ...
    - array/function to pointer type (如果T是reference type ```const T&```, 不会有convert to pointer)
...
- template matching rule:
    ...
    - 注意string literal types```"Hi"``` is const char array, 所以有```const T&a``` 和 ```const T*a``` 都avaiable 优先不call pointer的, 因为不需要 array to pointer conversion 
...
- specialization 
    ...
    - class template 可以 partial specialized , 在class 名字后的 ```<>``` specify template parameter we are specializing, 与原模板中 的参数 按位置对应.
...
//因为 "hi" 是 const char array, call pointer need conversion to pointer

```

E.g 7 如果同时 ```T&&``` 和 ```const T&``` 对于lvalue 都available, call ```T&&```, 因为更specialized. 但对于 const rvalue reference, call  ```const T& t```, 因为```T&&``` 需要const version， 不如```const T&```
...
<span style="background-color:#FFFF00">如果有tuple size 不知道, 可以用 ```decltype```</span>
```c++
...
- Like, bitset has <span style="color:red">**fixed size**</span> size must be constant expression： how many bits bitset contains 
...
- ```set```, ```reset```, ```flip``` change state of bitset 是 overloaded, <span style="color:red">如果no applies, the given operation to entire set</span>
...
| ```regex_replace``` | Replaces a regulare expression using a given format |
| ```sregex_itearotr``` |  Iterator adaptor that calls ```regex_search``` to iterate through the matches in a <span style="background-color:#FFFF00">**string**</span> |
...
| syntax | description | 
|----------|:-------------|
| ```(seq, m, r)``` <br/> ```(seq, m, r, mft)``` <br/>  ```(seq, r)``` <br/> ```(seq, r, mft)``` | 在character sequence ```seq``` 查找regex 对象 ```r``` 中的regular expression. ```seq``` can be <span style="background-color:#FFFF00">a **string**, a pirt of **iterators** denoting a range, or a pointer to a **null-terminated character array**</span>. ```m``` is a match object , 用来保存匹配结果细节. ```m``` and ```seq``` must have compatible types. ```mft``` is an optional ```regex_constant::match_flag_type``` value. (in §17.13) | 
...
下面表中 Constructor and assignment operations 也会会throw exceptions of type ```regex_error```
...
| ```r1 = re``` | 将```r1``` 中正则表达式替换为 ```re```. ```re``` 表示一个正则表达式, can be  <span style="background-color:#FFFF00">**string**</span>, a <span style="background-color:#FFFF00">pointer to a <span style="background-color:#FFFF00">**null-terminated character array**</span>, or a <span style="background-color:#FFFF00">**braced list of character**</span> | 
...
<center> Flag Specified when a ```regex``` is Defined: Defined in ```regex``` and ```regex::syntax_option_type```</center>
...
- Regulation Expression <span style="color:red">**语法正确**(sytactic correctness) 与否 evaluated at **run time**</span>
    ...
    - 就像stnadard exception types, ```regex_error``` 有 ```what``` operation desribte the error that occurred
...
| ```error_stack``` | <span style="background-color:#FFFF00">Insufficient memory</span> to evaluate a match |
...
| ```it1 == it2``` <br/> ```it1 != it2``` | 如果两个regex_ietator 都是<span style="background-color:#FFFF00">off-the-end iterator 则他们相等</span>. Two non-end iteartors are <span style="background-color:#FFFF00">**equal**</span> if they are <span style="background-color:#FFFF00">constructed from **the same input sequence and regex object**</span> |


跟上面例子一样查找所有的 ```[^c]ie``` 的pattern. 

- 当define ```it```, ```sregex_iterator``` constructor calls ```regex_seach``` to 定位 ```it``` on first match in ```file```. 
...
- One common **use for <span style="color:red">subexpressions**</span> is to validate data that must match a <span style="color:red">**specific format**</span>.
...
| ```first``` <br/> ```second``` | public data members that are iterators  to the start and one past the end of the matching sequence. <span style="background-color:#FFFF00">如果no match, then ```first``` and ```second``` are second</span> |
...
完整code: pattern 有7个subexpressions. 因此 ```smatch``` object conttain 8个 ```ssub_match``` elements. The element at ```[0]``` represent the overall match. The elements ```[1]...[7]``` represent each of the corresponding subexpressions. 
...
| Name | description | 
|----------|:-------------|
| Engine | Types that generate a sequence of random ```unisnged``` integers | 
...
- To get a number in a specified range, we <span style="color:red">use an object of a **distribution type**</span>:
    ...
    - 如果pass the engine object itself, ```u(e)``` (u is distribution, e is engine). 如果我们写成 ```u(e())```  含义变成, 将```e``` 生成**下一个值**传递给 ```u```, 会**compile-time error**.<span style="color:red"> We **pass the engine, not the next ersult of the engine**</span>, 因为some distribution may need to call the engine <span style="color:red">**more than once**</span>
...
use an object of a distribution type to geet a number in specified range. **注意只能写成** ```u(e)```, 不能是 ```u(e())```</span>
...
- 最常见但是 **Incorrect** way to obtain a random floating-point from ```rand``` is ```rand()/RAND_MAX``` (```RAND_MAX```: system-defined upper limit that is the **largest random number** that rand ```can``` return)
    - <span style="background-color:#FFFF00">错误原因是 **less precision**</span>: random integer 的精度 通常 低于 random floarting-point number, 因此<span style="color:red">some floating-point values that will never be produced as output</span>
...
生成结果为, 注: 如果打印出的图是完美对称的(perfectly symmetrical). 反倒有理由质疑randompnumber generator了
