...
- <span style="background-color:#FFFF00">函数主体有且只有一个return statement</span>(function body must contain exactly one return statement, 不能有通过if 判断的多个statement)
...
temp =  temp.set('#');
```
```const function```不能返回 nonconst的reference. 因为```*this``` is ```const Screen&```,```const Screen&``` 不能转换成```Screen&```. 所以```const```  function, 只能返回```const``` reference. 对于```const``` reference 不能call ordinary function (cannot conver from (const pointer to const) to (const pointer to nonconst) )
...
- 只允许一步conversion, 比如constructor take a string parameter, 我们不可以用```const char*``` 到string 再到class type
- **explicit**: prevents implicit conversion. <span style="background-color:#FFFF00">explicit meaningful only on constructor that can be called with a single constructor.</span>.
    - <span style="color:red">**explicit** is used nly on constructor declaration inside the class.</span> It is not repeated on a definition outside class body.  
...
e.g. ```string -> Sales_data```, compiler automatically creats a ```Sales_data``` object from the given ```string``` which pass to ```combine```. 因为```combine``` take const reference, we can pass tempoary to const reference
```c++
struct Sales_data {
    Sales_data(std::string s): Sales_data(s, 0,0) {} 
    Sales_data(std::istream &is): Sales_data() {read(is, *this); }
...
**Class-type are not always useful**: <span style="color:red">More problematic is the conversion from istream to Sales_data</span>. The code 转换```cin``` to ```Sales_data```, 创建的```Sales_data```是temporary. We have no access to it 一旦```combine```finishes. 当```item``` 完成后, ```temporary``` 会被丢弃
```c++
...
可以用 有explicit constructor to force conversion, ```static_cast``` to perform an explicit, rather than an implicit conversion. 用```static_cast``` uses the ```istream```constructor to construct temporary ```Sales_data``` object 
```c++
...
- It has no base classes or virtual functions. (没有base 和 virtual)
...
Aggregate class 的Intialization **Drawbacks**: 
1. Requires that all the data members of class be <span style="color:red">**public**</span>
...
```constexpr``` parameter 和 return type 必须是 **literal type**. 除了arithmetic types, references, and pointers, certain classes 也是**literal types**,  class that are **literal type** 也许有funcition members that are **constexpr**(需要meet all requirements of ```constexpr``` function, 这些function 也是<span style="color:red">**implicitly const**</span>)

- 一个**aggregate class** whose data members are of literal type is a literal class. 
- nonaggregate class 需要满足下列要求: 
  ...
  - The class must use default definition for its destructor (default constructor), which is the member that destroys objects of the class type.
