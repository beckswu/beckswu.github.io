...
- input and output function 必须是 <span style="background-color:#FFFF00">**nonmember function**</span>. 不能是member of own class. 因为如果是class member, 必须属于istream or ostream 类, 但是those class 是part of standard library, 不能加members to IO class  
...
- When we write ```point->mem```, <span style="background-color:#FFFF00">```point``` must be a **pointer** to a class</span> object or it must be an object of a class with an overloaded operator```->```. 根据point类型不同, writing ```point->mem``` is equivalent to
    1. ```(*point).mem;```. point is a built-in <span style="background-color:#FFFF00">**pointer**</span> type 
        - 先 dereference point and fetch from resulting object.  如果```point```没有 member named ```mem```, code is in error 
    2. ```point.operator()->mem;``` point is an object of <span style="background-color:#FFFF00">**class**</span> type
        - 如果 ```point``` is an object of a class that defines ```operator->```, 则使用 ```pointer.operator -> () ``` 来fetch ```mem```. 如果result is pointer, 重复step 1. 如果result is overloadede ```operator->()```, this step is repeatede on that object. process continues 直到 indicated member is return or some error 


值得注意
-  <span style="color:red">**deference**</span> 返回时 <span style="color:red">**reference**</span>, 而 <span style="color:red">**arrow**</span> 返回是 <span style="color:red">**pointer**</span>
...
-  overload the <span style="background-color:#FFFF00">**call operator**</span> allow objects of its type to be used as <span style="background-color:#FFFF00">**a function**</span>
    - 可以<span style="color:red">store state, **more flexible**</span> than ordinary functions.
    - <span style="color:red">**Objects**</span> of classes that define the <span style="color:red">**call operator**</span> are referred to as <span style="background-color:#FFFF00">**function objects**</span>.
- The **function-call operator** must be a <span style="background-color:#FFFF00">**member function**</span>, can <span style="color:red">define multiple versions of the call operator</span>, each of which must <span style="color:red">differ as to the **number** or **types** of their **parameters**</span>.
...
- **Lambdas Are Function Objects**
    ...
    - When we write a lambda, the compiler translates that expression into an <span style="color:red">**unnamed object** of an **unnamed class**</span> . <span style="color:red">The classes generated</span>  from a lambda <span style="color:red">contain an **overloaded function-call operator**</span> (parameter list and function body are the same as lambda)
        - by default, <span style="color:red">**lambdas** may not change their captured variables</span>.As a result, by default, the function-call operator in a class generated from a lambda is a <span style="color:red">const member function</span>. If the lambda is declared as <span style="color:red">mutable</span>, then the call operator is <span style="color:red">not const</span>
    ...
    - 当lambda <span style="background-color:#FFFF00">**captures by value**</span> are copied into the lambda. As a result, <span style="color:red">classes generated</span> from lambdas that capture variables by value<span style="color:red"> have data members corresponding to each such variable</span>. These classes also have a <span style="color:red">**constructor** to **initialize** these data members</span> from the value of the captured variables. 
...
- 解决上面的问题可以用 ```funtion``` defined in ```functional``` header
  ...
  - We cannot (directly) store the name of an **overloaded function** in an object of type function. 比如说一样的名字，但是属于不同类
...
- Compiler 只能apply 一步conversion at a time, 但是<span style="color:red">**implicit user-defined conversion** can be preceded or followed by a standard conversion</span>(built-in conversion，而 ```const char*``` 到 string 不算built-in) 
...
Although the compiler will apply only one user-defined conversion at a time. an implicit user-defined conversion can be preceded or followed by a standard (built-in) conversion. 比如下面例子 可以<span style="color:red">pass any arithmetic type to the SmalInt constructor</span>.  <span style="color:red">也可以将SmallInt 转换成int 然后 convert int 到其他arithmetic type</span>
...
Because conversion operators are implicitly applied, there is no way to pass arguments to these functions. Hence, conversion operators 不能有parameter. 尽管也不能提供return type, conversion function must return a value of its corresponding type:
...
一些早期的版本bool 算arithmetic type (所以转换成bool 又转换成 成arithmetic type ), 所以class 可以convert to bool (not explicit) 有意想不到的后果, 比如```ifstream``` had convertion to bool. 尝试用 output operator on an input stream, 因为没有 ```<<``` 定义 给 ```istream```, <span style="color:red">应该有error</span>, 但是use bool conversion operator to <span style="color:red">**convert cin to bool. 导致 bool to int**</span>. 所以 42 会shift left 1 or 0 
```c+=
int i = 42;
...
- If a class<span style="color:red"> has one or more conversions</span>, it is important to ensure that there is <span style="color:red">only one way to convert</span> from the class type to the target type. 
    - 两种方式multiple conversion paths: 第一种provide mutual conversion (<span style="background-color:#FFFF00">**converting constructor and conversion operator**</span>), 比如 A -> B, B -> A. 第二种是:  <span style="background-color:#FFFF00">**define multiple conversions**</span> from or to types that are themselves related by conversions. 比如 定义了 A -> int, A -> double, 那么 A —> double, 可以是一步conversion from A, 也可以是 A -> int -> double, <span style="color:red">不能用**cast** 来解决</span>, have the same ambiguity
    - 如果定义了多种方式conversions, <span style="color:red">之所以ambiguity, 因为conversions have the **same rank**</span>.  <span style="color:red">the **rank** of the standard conversion</span>, if any, preceding or following the conversion function, is used to <span style="color:red">select the best match</span>
- **Caution: Conversions and Operators**
  - Ordinarily, it is a <span style="color:red"> **bad idea**</span> to define classes with <span style="color:red"> **mutual conversions**(define mutually converting classess)</span> or to <span style="color:red"> define conversions to or from two arithmetic types</span>.  
    - 比如 ```Foo``` has a constructor from ```Bar```, 不要给 ```Bar``` conversion operator to ```Foo```
  - <span style="background-color:#FFFF00">Avoid conversions to the **built-in arithmetic types**</span>. 如果定义了 conversion to an arithmetic type, then
    - <span style="color:red">Do not define overloaded versions</span> of the operators that take arithmetic types. 比如 定义了 int to ```Foo```, 不要定义```double``` to ```Foo```
    - <span style="color:red">Do not define a conversion to more than one arithmetic type</span>. Let the standard conversions provide conversions to the other arithmetic types. 比如定义了 ```Foo``` to ```int```, 不要定义 ```Foo``` to ```double```
  - **The easiest rule of all**: With the exception of <span style="background-color:#FFFF00">an **explicit** conversion to bool</span>, <span style="color:red">avoid defining conversion functions and limit nonexplicit constructors</span> to those that are “obviously right.”
- Needing to <span style="color:red">use a constructor or a **cast** to **convert an argument**</span> in a call to an <span style="color:red">**overloaded function**</span> frequently is  **bad design**</span>. 注意下面例子
  - <span style="background-color:#FFFF00">Call to an **overloaded function**</span>, if two (or more) <span style="color:red"> **different** user-defined conversions provide a viable match</span>(比如是 ```short``` to ```Foo``` 还是 ```double``` to ```Bar```时候), the conversions are considered equally good ambiguity, <span style="background-color:#FFFF00">**rank** of any standard conversion 不会被考虑</span>. 只有当**同一个** user-defined conversion时候(比如是 ```short``` to ```Foo``` 还是 ```double``` to ```Foo```时候), **rank** of standard conversion 才 被考虑


下面例子obtain A 可以从 A的constructor or B's conversion operator. <span style="color:red">the compiler doesn’t know which conversion to run</span>; the call to f is <span style="color:red">**ambiguous**</span>. the call is in <span style="color:red">**error**</span>. 不能用 **cast** 来解决ambiguity
```c++
...
<span style="color:red">**Ambiguities**</span> also occur when a class <span style="color:red">**defines multiple conversions** to (or from) types that are themselves related by conversions</span>. 比如下面例子定义constructor from two arithmetic types and conversion operators to two different arithmetic types. 
...
Choosing among multiple conversions is further complicated when we call an **overloaded function**. 如果两个或多个类型转换都提供了同一种可行的匹配，这些类型转换一样好. 下面例子<span style="color:red">**ambiguity**</span> 因为<span style="color:red">overload functions take parameters 的class type  定义了一样的 converting constructor </span>
...
- The <span style="color:red">set of candidate functions</span> for an operator used in an expression can <span style="background-color:#FFFF00">contain both nonmember and member functions</span>.
- When we <span style="color:red">call a **named function**</span>, <span style="color:red">**member** and **nonmember functions** with the same name do not overload</span> one another, 因为syntax 不同(call member type 需要through object, reference or pointer of such object). 但是When we use an <span style="color:red">**overloaded operator** </span> in an expression, there is <span style="color:red">nothing to indicate</span> whether we’re <span style="color:red">using a **member** or **nonmember** function</span>. 
...
**Inheritance**:
- a **base class** at the root of the hierarchy, from which the other classes inherit, directly or indirectly. These inheriting classes are known as **derived classes**. 
  - base class: 定义 common members. derived clas define memeber that are specific to derived class
- A derived class must specify the class(es) from which it intends to inherit. It does so in a class <span style="color:red">**derivation list**</span> (继承了哪个class, 是public, private, protected):  a colon followed by a comma-separated list of base classes each of which may have an optional access specifier
- 在 derived class **virtual** function, <span style="color:red">keyword **virtual** 加不加都可以</span>
- new standard lets a derived class explicitly note that it intends a member function to<span style="color:red"> **override** a virtual that it inherits</span>. It does so by <span style="color:red">specifying override after its parameter list, or after function **const**  keymember or after **reference qualifier**</span>
...
- When we <span style="color:red">call a **virtual function** through a **pointer** or **reference**</span>, the <span style="color:red">call will be **dynamically bound**</span>. Depending on the type of the object to which the reference or pointer is bound, the version in the base class or in one of its derived classes will be executed.
-  <span style="background-color:#FFFF00">Any **nonstatic member** function, other than a constructor, 可以是 virtual</span>. <span style="color:red">**virtual** 只能用于the declaration inside the class</span>, outside class定义时不能用
    - virtual function in base class is  <span style="color:red">**implicitly virtual** in derived classes</span>
...
- 尽管derived class contians member from inheritance from base, 但<span style="color:red">cannot directly initialize those members</span>. a <span style="color:red">derived class must use a **base-class constructor** to initialize</span> its base-class part. Base-class part initialized along witht data members of derived class, during the initialization phase of the constructor 
    ...
    - The base part of a derived object is default initialized,  To use a different base-class constructor, we provide a constructor 在derived 的 constructor initializer list
...
- <span style="color:red">A class must be **defined**, not just declared, before we can use it as a **base class**</span>: 因为derived class 可能contains and use memeber it inherits from base class. 使用这些 members, derived lass must know what they are. 所以重要的implication是: <span style="color:red">impossible to derive a class from itself</span>
- 可以Preventing Inheritance:通过定义 class as **final**: ```class NoDerived final ```

<span style="background-color:#FFFF00">**derived-to-base** conversion</span>.  the compiler will <span style="color:red">apply</span> the derived-to-base conversion <span style="background-color:#FFFF00">**implicitly**</span>.  We can <span style="background-color:#FFFF00">use an object of derived type or a **reference** to a derived type</span> when a <span style="color:red">**reference to the base type**</span> is required. Similarly, we can use <span style="color:red">a pointer to a derived type</span> where a <span style="color:red">**pointer to the base type**</span> is required  下面例子可以用reference 去access derived type 上面function 例子也是take reference as parameter
...
一个class 可以同时是base class, 也可以是 derived class. 下面例子 Base is a **direct base** to D1 and an **indirect base** to D2. A **direct base** class is named in the derivation list. An **indirect base** is one that a derived class inherits through its direct base class.  **The most derived object** contains a subobject for its **direct base** and for each of its **indirect bases**.
```c++
classBase{/* ... */};
...
- Ordinarily, we can <span style="color:red">bind a reference or a pointer</span> only to an object that has <span style="color:red">the same type as the corresponding reference or pointer</span> or to a type that involves an acceptable <span style="color:red">const conversion</span>
    - class type 有个例外: <span style="background-color:#FFFF00">bind a **pointer or reference** to a base-class type to an object of a type **derived** from that base class</span>
        - derived to base conversion 之所以存在因为没诶一个derived class 都包含base-class part (用base class type 的reference or pointer 可以绑定到上面)
...
- <span style="color:red">**static type**</span> of an expression: <span style="color:red">known at **compile time**</span> : it is the type with which a variable is declared or that an expression yields. The <span style="color:red">**dynamic type** is the type of the object in **memory**</span> that at the variable or expression represents. The dynamic type may <span style="color:red">not be known until **run time**</span>.
...
- The automatic <span style="color:red">**derived-to-base conversion** applies only for conversions to a **reference** or **pointer** type</span>. 没有conversion from a derived-class type to the base- class type. 但是possible convert an object of a derived class to base-class type. However, such conversions may not behave as we might want.
...
对于base class might or might not be part of a derived object, there is <span style="color:red">no automatic **conversion from the base class to its derived class(s)**</span>: 下面例子 attempt to use ```bulkP``` or ```bulkRef``` to use members that do not exist in **base**
...
cannot convert from base(a base pointer or reference is bound to a derived object) to derived. 因为compiler no way to know that a specific conversion is safe at run time. Compiler 仅仅look at static types of pointer or reference to 判断if conversion legal. <span style="color:red">如果base class 有one or more virtual functions, can use ```dynamic_cast```</span> (cover at 19.2.1) to request a conversion that checked at run time.  <span style="color:red">如果当我们知道conversion is safe from base to derived, 可以用 ```static_cast``` override the compiler</span>
...
当我们initialize or assign an object of class type, initialize 可以用constructor, assign calling an assignment operator. 这些member has parameter that is <span style="color:red">const reference to class type</span>. 因为是reference, the **derived-to-base conversion** lets us pass a derived object to a base-class copy/move operation; 这些operation not virtual. When we pass a derived object to a base-class constructor/assignment operator, <span style="background-color:#FFFF00">**constructor/operator knows only about the member of the base class**</span>. 比如我们定义的class, 有synthesized versions of copy and assignment. That constructor only knows ```Quote``` 的 ```bookNo``` 和 ```price```. <span style="color:red">copies those member from ```Quote``` and ignore the members that are part of the ```Bulk_quote``` </span>

```c++
Bulk_quote bulk; // object of derived type

Quote item(bulk); // uses the Quote::Quote(const Quote&) 

constructor item = bulk; // calls Quote::operator=(const Quote&)
...
-  <span style="background-color:#FFFF00">**virtual functions** must always be define</span>. <span style="color:red">因为**dynamic binding**</span> happens when a **virtual** member function is called through a **reference** or a **pointer** to a base-class type. 不知道call 哪个function **until runtime**.  <span style="background-color:#FFFF00">通常上, 如果我们不需要哪个function, 无须定义</span>. 
-  When we call a virtual function on an expression that has a <span style="color:red">plain—nonreference and nonpointer—type</span>, that <span style="color:red">call is bound at **compile time**</span>.
- The key idea behind OOP is **polymorphism**(from Green meaning "**many forms**"). inheritance as **polymorphic** types. we can use the “many forms” of these types while <span style="color:red">ignoring the differences among them</span>. The fact that the **static** and **dynamic** types of references and pointers can differ is the cornerstone(根本所在) of how C++ supports polymorphism.
  - 比如使用 reference or pointer to the base class, 不知道type(base type or derived type) until runtime. 如通过 pointer or reference call virtual function. The version of  virtual function 由 the reference or pointer 绑定对象的真实类型决定
  - On the other hand, <span style="background-color:#FFFF00">calls to **nonvirtual functions**(pointer/reference) are bound at **compile time**</span>. <span style="background-color:#FFFF00">calls to any function (virtual or not) on an **object**(static: The type of an object is fixed and unvarying)</span> are also bound at **compile time**.
- 不需要在derived class 再次mention virtual,  <span style="color:red">Once a function is declared as ```virtual```, it remains ```virtual``` in all the derived classes.</span>. derived class override inherited virtual function <span style="color:red">必须有一样的**parameter type** as the base-class function </span>
  - return type 一般也要match, 一个例外是 <span style="background-color:#FFFF00">return a **reference** / **pointer**</span> to types that are themselves  <span style="color:red">related by inheritance</span>.  比如 ```D``` 继承 ```B```, B 的virtual 返回 ```B*```, D的继承的virtual 可以返回 ```D*```,  such return types require that the <span style="color:red">**derived-to-base conversion** from D to B is accessible</span>
- 可以定义a function in derived class with the same name as virtual in base class 但是 <span style="color:red">不同的 parameter list</span>. Compiler 会认为这个function 是 independent from base-class function (不是override base class virtual function). 
  - 可以用```override``` to fix 上面的issue, <span style="color:red">The compiler will **reject**</span> a program if a function <span style="color:red">marked ```override``` does not override</span> an existing virtual function
  - overrride 只能用于 virtual function
  - 把function 定义成 virtual 表示不能再override ride, 比如 C 继承 B， B继承A, B override 的virtual from A as final 表示 C中不能再override 这个function
  - final 和 override 都在 parameter list (包括了 const 和 reference qualifier ) 之后
- Virtual functions that have <span style="color:red">**default arguments** should use the same argument values in the base and derived classes</span>.
  - default argument bind at compiled time, when a call is made through pointer or reference to base,  the default argument(s) will be those defined in the base class, 如果derived class 提供了不一样的argument, 并不会用到
- 有些情况<span style="background-color:#FFFF00">prevent dynamic binding</span> of a call to virtual function, 可以force call to use 特定的virtual(不用derived 用base的), 可以用scope operator to do so. This call will be <span style="background-color:#FFFF00">**resolved at compile time**</span>.
...
#### (d). Access Control and Inheritance
...
- A <span style="color:red">**derived class member**</span> or <span style="color:red">**friend**</span> may access the <span style="color:red">**protected members of the base**</span> class only through a <span style="color:red">**derived object**</span>. The derived class has no special access to the protected members of base-class objects.
-  **derivation access specifier** has no effect on whether members (and friend) of derived class access member of **direct base class**. It is to <span style="color:red">control the access that users of the derived class</span>  -including other classes derived from the derived class—have to the members inherited from Base.
    - 如果是protected inherited, derived class的 member and friend 可以使用 base 的 public/protected, 不能访问base 的private,  但是user 不能使用public 的public/protected
...
下面的friend 定义在derived 中 access base member 的protected member 只能用derived class object 而不能用base class object, 原因是friend 定义在derived 中 而不是定义在base 中. 如果可以access base class 的 protected member 通过base class object, 就即使不是friend 也有更改它的风险, 所以prevent such usage, 禁止这么做
