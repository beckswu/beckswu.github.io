...
- **Unnamed Namespaces**: 在<span style="color:red">keyword ```namespace``` 后 没有name</span>, 后紧跟curly braces 括起来的 declarations
    ...
    - Names defined in an unnamed namespace are in the **same scope** as the scope at which the namespace is defined. 如果unnamed namespace 中member name 需要区别于unname scope  enclosing scope的 member names.  注意下面例子
