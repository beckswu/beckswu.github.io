...
- 不能在const function中call nonconst member function, 因为<span style="background-color:#FFFF00">Cannot bind ```this```（this 没有low level const） to a ```const``` object (```this```没有lower const, lower const -> no lower const的 不可以) </span>. 
...
- const function 中只能call const 的function 
- const object 只能call const function 不能call nonconst function, <span style="background-color:#FFFF00">**Objects** that are const, and **references** or **pointers** to const objects, may call only const member functions.</span> 
...
- <span style="background-color:#FFFF00">**static members**</span> of a class exist <span style="background-color:#FFFF00">**outside any object**</span>.Object do not contain data associated with static data members. static member functions 不与对象绑定(bound)在一起.  
