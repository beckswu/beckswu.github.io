...
- Classes control default initialization by defining a special constructor, known as the **default constructor**: is one that takes no arguments.
  - default constructor 是constructor work without any argument; 错误的概念是constructor with no parameters (either no parameters, or all parameters have default values); 比如```dog(string name = “Bob”); ```属于can work without any argument, 但有default parameter
