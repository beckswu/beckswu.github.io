...
1. 对于reference, pointer (non const) 等号两边类型必须相同， 比如 double dval = 3.14;,int &refVal5 = dval;
2.  non-const initialize 等号左右两边类型必须相同（因为reference 不是object） 
3. 对于const 非reference, 左右两边等号类型可以不同. 
4. **对于const reference 等号两百年可以不同类型，compiler会做转换**  const double pi = 3.14; const int & i = pi; //okay
