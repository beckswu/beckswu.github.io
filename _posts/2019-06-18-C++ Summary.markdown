...
- Classes control default initialization by defining a special constructor, known as the **default constructor**:  <span style="background-color:#FFFF00"> is one that takes no arguments.
  - </span> default constructor 是constructor work without any argument; 错误的概念是constructor with no parameter; 比如```dog(string name = “Bob”); ```属于can work without any argument, 但有default parameter
  - 如果没有explicitly define any constructors, <span style="color:red">compiler will implicitly define and generate the default constructor</span> known as the <span style="background-color:#FFFF00">**synthesized default constructor**</span>, 对于大多数class, this synthesized constructor initializes each data member as follows: 
    - 如果there is an in-class initializer, 用它initialize member(如果存在class的初始值,用初始值初始化成员) 比如class 中```string a = "dog";```
  ...
  - 如果compiler 支持in-class initializer (在class 内部```int a = 1;```) 我们可以用default constructor without parameters, 如果不支持, your default constructor should use the **constructor initailizer list** to initialize every member of the class (e.g. ```sale(): a(0) {}```)
