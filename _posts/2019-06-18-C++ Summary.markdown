...
- After Options that add elements: (Iterators, pointers, and references aka IPR)
  ...
  - 对于 **deque**, add elements 除了首尾任何位置, IPR都invalid; <span style="background-color:#FFFF00">在 front or back, Iterators are invalidated, but references and pointers to existing elements are not</span>
...
- After we remove element
  ...
  -  对于 **deque**, remove elements 除了首尾任何位置 IPR都invalid; <span style="background-color:#FFFF00">如果删除是back, off-the-end itertor is invaldiated 但是其他的IPR 都有效 （unaffected）. 如果删除是begin, 所有都unaffected. </span>
