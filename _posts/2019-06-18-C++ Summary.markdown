...
1. reference, pointer (non const):
    - 等号两边类型必须相同, 比如 double dval = 3.14;,int &refVal5 = dval;//error
    - cannot assign 底层const, 可以assign 顶层const, (<span style="background-color: #FFFF00">因为拷贝对象时候底层const 不能被忽略，顶层可以忽略</span>, 详见**Pointer and const**) 比如 
