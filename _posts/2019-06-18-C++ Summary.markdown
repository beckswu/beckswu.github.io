...
- ```const``` is to modify the type of the implicit ```this``` pointer, 放在parameter list之后表示 ```this``` is a pointer to ```const```. member function that use ```const```被称为<span style="color:red">const member functions</span>
...
- 尽管```this``` is implicit, 但是follow initialization rules, which means cannot bind ```this```（this 没有low level const） to a ```const``` object(比如 pointer to const value).  因为我们cannot call an ordinary member function on a const object; 
...
**Constructors** 
- constructor is run whenever an object of a class type is created.只要class的对象被创建, 就会执行constructor
...
- Classes control default initialization by defining a special constructor, known as the **default constructor**.  <span style="background-color:#FFFF00">constructor 不能被declared as const. The default constructor is one that takes no arguments.</span> default constructor 是constructor work without any parameter; 错误的概念是constructor with no parameter; 比如```dog(string name = “Bob”); ```属于can work without any parameter
  ...
  - <span style="background-color:#FFFF00">The compiler-generated constructor is known as the **synthesized default constructor**</span>, 对于大多数class, this synthesized constructor initializes each data member as folows: 
    ...
    - Otherwise, default-initialze the member(否则, 默认初始化成员)
...
- Friend declarations may appear only inside a class definition. 友元定义只能在class 内部 (好的编程习惯是: 集中在程序开始或者结尾集中定义友元)
- <span style="background-color:#FFFF00">friend declaraction 仅仅specifies access, 而不是general declaration of the function</span>, <span style="color:red">如果我们想要users able to call friend function, 需要declare the function separately from the friend declaration</span>. 让firend visible to users, we usually declare each friend (outside the class) in the same header as the class itself. 
...
- A class 必须被defined 而不仅仅是声明before we write code that creats objects of that type. 以你为compiler 需要know how much storage such objects need. 同理, a class 必须被定义before a reference or pointer is used to access a member of the type. 如果不定义, compiler cannot know 它有什么member
...
Sales_data item1; // equivalent declaration
