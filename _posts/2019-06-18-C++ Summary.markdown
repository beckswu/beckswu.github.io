...
- When we write ```point->mem```, <span style="background-color:#FFFF00">```point``` must be a **pointer** to a class</span> object or class with an overloaded operator```->```. 根据point类型不同, writing ```point->mem``` resolve 步骤是:
    1. ```(*point).mem;```. point is <span style="background-color:#FFFF00">**pointer**</span> type 
    2. ```point.operator->()mem;``` point is an object of <span style="background-color:#FFFF00">**class**</span> type
        - 如果 ```point``` is class that defines ```operator->```, 则使用 ```pointer.operator->() ``` 来fetch ```mem```. 如果result is pointer, go step 1. 如果result 不是poitner, 继续step2 直到fetch的是pointer type or some error 
