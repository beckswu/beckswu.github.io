...
**Overloading Operators**: the type of operands(运算对象的类型) and the result 取决于h how the opeartor is defined. 但是the number of operands and ther precedence(优先级) and the associativity (结合律) of the operator cannot be changed (运算对象个数, 优先级, 结合律不能改变)
...
- 具体使用: 
  - Assignment 等号左面需要(nonconst) lvalue as its left-hand operand nad yields its left-hand operand as lvalue 
  ...
  - Built-in dereference and subscript operators(vector, string), iterator derference all yield lvalue 
  - Built-in 和 iterator 的 increment, decrement operators 也需要lvalue as operands
    - prefix operators (```++i```) return the object itself as lvalue, postfix```i++``` return a copy of the object's original value as rvalue
...
Precedencesspecifies how the operands are grouped 但是 <span style="background-color:#FFFF00">say nothing about the order in which the operands are evaluated.</span>  比如: ```int i = f1() * f2(); ``` 我们知道```f1``` and ```f2``` must be called before 乘法. 但是no way of knowing whether ```f1``` 先执行还是 ```f2``` 先执行

**准侧**: 1. 当不确定时, 用括号, 2. if you change the value of an operand, don't use that operand elsewhere in the same expression. (Exception for rule2: when the subexpression that changes the operand is itself the operand of another subexpression, 当更改是在本身做的更改, ```*++iter```, ```++iter``` 更改了```iter```本身, 再用deference operator, no issue. The increment must be evaluated before dereference)
...
The compiler might evaluate ++i before evaluate i (1,1) or compiler might evaluate i then evaluate ++1 (0,1)

比如下面的, no guarantees which functin are called first. 如果下面的function 是独立的 and do not affect the state of the same objects or perform IO, then it is okay. 如果functions do affect the same object. then expression in error and has undefined behavior.
```c++
f() + g() * h() + j()
```
...
**overflow** happens when a value is computed that is outside the range of values. 比如shorts are 16 bits. maximum short is 32767. 比如下面例子, the value wrapped around. The sign bit which had been 0 is set to 1, resultng in a negative value. 在其他system, the result might be different(也许会crash entirely).  On many systems, there is no compile-time or run-time warning when an overflow occurs. 
...
- Assignment has low precedence, 所以想要先赋值, 用括号
...
- <span style="background-color:#FFFF00">Operands can be evaluated at any order</span> .e.g when one subexpression changes the value of an operand that is used in another subexpression. 
...
int ival, *pval;
ival = pval = 0; //error: they are different type, 

//and no conversion from int * to int  

```
因为assignment 是right associative, the right-most assignment ```jval = 0```  the result of the right-most  assignment ```jval``` is assigned to ```ival```， multiple assignment must have the same type. 

Assignment has low precedence, 下面例子如果不用括号, the operands to ```!=``` would be evaluate first, then true/false assign to i
```c++
int i;
...
对于compound assignment, the left-hand operand is evaluated only once. 对于谱图的the operand is evaludated twice: 第一次是右侧的expression, 第二次是赋值给左侧
...
The problem is both left- and right-hand operands use beg and right-hand operand changes beg. The assignment is undefined
...
The precedence of postfix increment 高于deference ```*pbeg++``` is equlalen to ```*(pbeg++)```, ```pbeg++``` increment ```pbeg``` and return a copy of previous value as rvalue 
...
cout<<*pbeg++<<endl;//printthecurrentvalueandadvance pbeg

```

derference has a lower precedence than dot, 如果我们忽略了括号, ```*p.size()``` error 因为pointer doesn't have member size. <span style="background-color:#FFFF00">The arrow operator yields an lvalue. The dot operator yields an lvalue if the object from which the member is fetched is an lvalue; otherwise the result is an rvalue. </span>
...
The conditional operator has fairly low precedence. When we embed a conditional expression in a larger expression, 通常需要parathesize 
