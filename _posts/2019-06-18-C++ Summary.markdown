...
1. 对于reference non-const initialize 等号左右两边类型必须相同（因为reference 不是object） 
2. 对于const 非reference, 左右两边等号类型可以不同. 
3. **对于const reference 等号两百年可以不同类型，compiler会做转换**  const double pi = 3.14; const int & i = pi; //okay
4. 对于non-const pointer, reference, 不能initialize pointe to a const value 比如const double pi = 3.14;  double \* ptr = &pi; wrong; double &ref = pi; //wrong
5. 
