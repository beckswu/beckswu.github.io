...
- Each namespace is a **scope**. Different namespaces introduce different scopes, different namespaces may have members with the same name
    - Names defined in namesapce 可以被该namespace内成员 直接访问, 当被该namespace 外访问时 must indicate the namespace (which name is defined)
...
- **Namespaces can be discontiguous**: a namespace 可以被定义 in several parts
    ...
    - definitions of namespace member can be put in separate source files
        ...
        - 但不可以define in unrelated namespace
    - 这么定义不会造成any compile-time or link-time errors.
...
- **Unnamed Namespaces**: 在<span style="color:red">keyword ```namespace``` 后 没有name</span>, 后紧跟curly braces 括起来的 declarations
    ...
    - Names defined in an unnamed namespace are  <span style="color:red">**used directly**</span>. 也找不到namespace name which qualify them. <span style="color:red">not possible to use the scope operator to refer to members of unnamed namespaces</span>
    - Names defined in an unnamed namespace are in the **same scope** as the scope at which the namespace is defined. 如果unnamed namesapce defined 在 outermost scope in file, then <span style="color:red">names in the **unnamed namespace** must **differ from** names defined at **global scope**</span>.  注意下面例子
...
- Using Directive 和 Using Declaration 之处是, namespace 中所有 名字都是 <span style="color:red">visible</span> without qualification
- Using Directive 格式是 : ```using namespace namespace_name```
    - It is <span style="color:red">**error**</span> 如果用 namespace name not previously defined
...
- <span style="color:red">**undeclared class** or **function**</span> that is first named <span style="color:red">in a friend declaration</span> is assumed to be a <span style="color:red">member of the **closest enclosing namespace**</span>。 一个未声明的类或者函数 第一次出现在右元声明中, 则认为它是最近外层命名空间的成员
...
- using declarations:
    ...
    - <span style="color:red">using declaration 引用functions 将overload any 一样名字 functions</span> declartions in the scope where using declaration appears
...
<center> Conceptual Structure of a Panda Object </span>
...
1. ```Bear``` initialized first
2. ```Endangered``` second direct base is initialized next 
3. ```Panda``` most derived part is initialized last
4. ```Zoo``` 最后
...
pb->print(); // ok: Panda::print() pb->cuddle(); // error: not part of the Bear interface pb->highlight(); // error: not part of the Bear interface delete pb; // ok: Panda::~Panda()
...
-  **virtual inheritance** lets a class specify that it is willing to <span style="color:red">share its base class</span>. 
