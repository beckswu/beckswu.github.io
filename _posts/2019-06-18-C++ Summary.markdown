...
- When a <span style="background-color:#FFFF00">**derived class**</span> defines a <span style="background-color:#FFFF00">**copy** or **move** operation</span>, that operation is responsible for copying or moving the entire object, including <span style="color:red">**base-class** members</span>
    - 当用derived synthesized copy constructor 会call base (synthesized or defined) constructor 
    - 当用derived defined copy constructor 但没有call base copy constructor, 会call base default constructor，因为没有explicitly 指出, 所以B 的copy constructor 会先initialize base member using base default constructor. 
    - 当用derived defined copy constructor, <span style="color:red">**explicitly call** base copy constructor</span> putting in <span style="color:red">derived’s copy constructor **initializer list**</span> 这样initialize base member using base copy constructor. 
...
D 的assignment operator, 注意因为是<span style="color:red">self-define assignment operator 并不会自动call base class synthesized/self-define assignment operator</span>,
```c++
