...
**inline vs constexpr**
- inline functions, expression are always evaluated at the <span style="color:red"run time</span>. 但是constexpr functions are evaluated at <span style="color:red">compiled time(not always)</span>
...
- 可以define static member function inside or outside of class body.如果在outside define, we don't need to repeat ```static``` keyword(static只需要声明时候加上)
...
- 向其他object一样,static data member只能被定义一次 (好习惯是 把所有static member的definitions 和所有noninlin member functions的定义放在一起)
...
- 因为static member 是不跟object 绑定, 所以static data member 可以是incomplete type. static data member 可以事the same type as the class type, A non-static member 是可以这样declared, 只可以declare reference or pointer to an object of its class
- we can use static member as default argument. (非static data member不能被使用为default argument 因为its value is part of the object, 我们无法提供一个真的对象从中获取成员的值, so is an error) 
...
- 因为IO 不能符号或者赋值, 所以 不能有parameter or return ytpe 是 IOStream types. 对IO操作的function <span style="background-color:#FFFF00">通常通过reference的方式pass and return</span>
...
- 一旦stream 发生错误, 后续的IO 操作都失败, 比如```int a; cin>>a``` 却输入了string. 简单方法是check the stream is okay before attempint to use, 下面的while <span style="color:red">check the state of stream returned from ```>>``` expression</spann> . 如果成功再继续
...
| ```sgood()``` |  return true if the stream s is in a valid state | 
...
- 当一个输入流关联到输出流, 任何试图从输入流(intput stream)读取数据的操作 都会先刷新关联的输出流(output stream). 
  - The library ties ```cout``` to ```cin```. 因此 ```cin >> ival``` 会导致```count```的缓冲区刷新(flush)
- interactive systems 通常应该关联输入流和输出流, 意味着, 用户提示的信息，都会在读操作前被打印出来

**tie**: has two overloaded versions. 
- one version takes no argument, 返回指向输出流指针. 如果this object is currently tie to output streams. 返回就是a pointer to the output stream。 另一个version: take a pointer to an ```ostream```, 将自己关联到```ostream```上, ```x.tie(&o)```: tie the stream ```x``` to the output stream ```o```. 
...
cin.tie(&cerr); // 读取cin 会刷新cerr 而不是count 
...
- when we creat a file stream, 可以选择性提供一个file namem, associate that object with file. 如果提供file name, ```open``` is called automatically.
...
- When an fstream object is destroyed, close is called automatically. 局部变量```fstream```离开作用域时, 关联文件会自动关闭
...
| ```fstrm.open(s)``` | 打开名为s的文件, 并将文件与```fstrm``` 绑定.  ```s``` 可以是string or pointer to C-style character string. 这个constructor 是```explicit```的, 文件默认模式depends on the type of ```fstream``` | 
...
- ```out``` 只能对```ofstream``` or ```fstream``` object
- ```in``` 只能对 ```ifstream``` or ```fstream``` object
-  只有当```out``` is specified, 才能设定```trunc``` 模式
-  只要当 ```trunc``` 没被设定, 就可以设定```app``` 模式. 如果指定```app```, 即使没有explicitly specify ```out```, 文件也是以输出方式打开
-  By default, ```trunc```is default mode for ```out```.如果要保留文件内容, 需要specify ```app```, or specify ```in``` mode which file is open for both input and output 
...
- **string** and **vector** old their elements in <span style="background-color:#FFFF00">contiguous memory</span>.由下表计算其地址很快, 但是一次插入或者删除后,<span style="background-color:#FFFF00">需要移动插入/删除位置之后的所有元素, 来保持连续存储(maintain contiguity)</span>; 而且添加一个元素可能需要分配额外的存储空间. At the case, every element must be moved into the new storage. 
...
- 如果程序需要random access , insertion and deletion. 决定取决于the relative cost of accessing the elements in a **list** or **forward_list** VS the cose of inserting or deleting elements in a **vector** or **deque**
...
- Can use constructor that takes a size argument if the element type is built-in type or a class type that has a default constructor 例如: ```vector<int> a(10)``` 默认初始化了10个0
...
- <span style="color:red">如果没有default constructor type, we cannot cosntruct such containers using only an element count</span>
- <span style="color:red">**Array**: size is part of its type</span>. 当define array, 需要specify element type and size ```array<int, 42>```. **Array** 不支持normal container constructor 因为这些constructor need the size of the container. 
    ...
    - array 支持copy assignment constructor, 只要type (size, element type) matches
    - array 不支持 braced list assignment ```array<int,10>a1; a1 = {0}; //error```
    - array 不支持 iterator constructor, Assign operations 
...
- 在library 有both and nonmember version of ```swap```，好习惯是使用nonmember version of ```swap```
...
**Iterators**
- ```forward_list``` 不支持 递减(--) operator. 也不支持```reverse_iteator```
...
**swap** : with the exception of string, iterators, references, and pointers into the containers are not invalidated. 比如had ```iter``` denoted the string at ```sevc1[3]``` before swap, swap之后, ```iter```指的是```svec2[3]```
```c++
...
- Adding (insert) elements to ```vector```, ```string```, ```deque``` 会invalidates all existing iterators, references, and pointer
...
- ```Array``` 和```forward_list``` 不支持```push_back``` 剩下都支持 (a copy of object)
- ```Array``` 不支持```insert```, 剩下都支持,(vector, deque, list, string, forward_list), Insert into a vector, deque, string is expensive operation 
- ```emplace(front, back)```construct elements in the container ( construct an element directly in space managed by the container在内存空间中直接构造元素). The argumentsmust match a constructor for the element type. 
- 包括```Array```在内所有成员都有```front```,只有```forward_list```没有```back```,因为不支持```reverse_iterator```, 也不能```--```递减```forward_list```的iterator, 但可以call ```forward_list```的```end()```
- The Access Members Return References (```back()```, ```frot()```, ```c[n]```, ```c.at(n)```), e.g. ```auto & v = c.back(); v = 1024;```, 
...
| ```c.erase(b,e)``` | b,e is iterator of range, 删除p,返回删除元素之后的元素的 iterator，如果e is end, return end iterator |
...
- After Options that add elements: (Iterators, pointers, and references aka IPR)
  ...
  - 对于 **deque**, add elements 除了收尾任何位置都invalid; <span style="background-color:#FFFF00">在 front or back, Iterators are invalidated, but references and pointers to existing elements are not</span>
  - 对于 **list**, **forward_list** IPR都 都有效
- After we remove element
  -  **list**, **forward_list** IPR 都 remain valid 
  -  对于 **deque**, remove elements 除了收尾任何位置都invalid; <span style="background-color:#FFFF00">如果删除是back, off-the-end itertor is invaldiated 但是其他的IPR 都有效 （unaffected）. 如果删除是begin, 所有都unaffected. </span>
...
- Avoid Storing the Iterator Returned from end 因为add / remove elements 总会把```vector``` or ```sttring``` 的```end``` iterator invalid
...
- 当create a string from a ```const char*```, <span style="color:red">array 必须是 null terminated.</span>, copy 遇到null 停止. or pass a count , array 可以不用以null 结尾
    - <span style="color:red">但是如果不pass count and no null, 或者given count 大于size of array, operation is undefined</span>
...
| Syntax | Description | 
| --- | :--- | 
| ```stoi(s, p, d)``` <br/>  ```stol(s, p, d)``` <br/>  ```stoul(s, p, d)``` <br/>  ```stoll(s, p, d)``` <br/>  ```stoull(s, p, d)```  | b表示numeric base for the conversion(几进制), p is a pointer to a ```size_t``` in which to put index of first non-numeric character(非数值的) in s` |
...
- container adaptor takes an existing containeer type and makes it act like a different type. 比如```stack``` 可以takes a sequential container (other than ```array``` or ```forward_list```. 
...
- 默认情况下, ```stack```and ```queue``` are implemented in terms of ```deque```, 而```priority_queue```是在```vector```上实现的
    ...
    - e.g. override default container type by naming a sequential container as a secnd type argument. ```stack<string, vector<string>str(svec)```
...
- 每一个container adpator defines 自己的操作类型 in terms of 底层容器的操作类型. 只能使用adaptor自己的操作类型,不能用underlying container type. 
