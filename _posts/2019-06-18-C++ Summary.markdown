...
- It is an <span style="color:red">**error** to use raw memory</span> in which an object has <span style="color:red">not been constructed</sapn>
...
| ```a.destroy(p) ``` | Runs the destructor on object pointed to by the the ```T*``` pointer p | 
...
| ```uninitialized_fill(b,e,t) ```  | Constructed objects in the range of **raw memory** denoted by iterators b and e as a copy of t (从b到e 值均为tt的拷贝) |
