...
| Syntax | Assignment Operations Description | 
| --- | :--- |
...
v1 == v4 // true; each element is equal and v1 and v4 have the same size() v1 == v2 // false; v2 has fewer elements than v1
...
- Adding elements to ```vector```, ```string```, ```deque``` 会invalidates all existing iterators, references, and pointer
...
| ```c.insert(p,t)``` <br/>  ```c.emplace(p,args)``` <br/> | p is iterator, 在p之前插入，返回iterator 指向新添加元素 |
