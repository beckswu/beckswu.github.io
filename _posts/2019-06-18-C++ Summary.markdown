...
陷阱: 1. constexpr, type Aliases under  [2. Variable and Basic Types](# 2. Variable and Basic Types) 2. Array: Pointer Arithmetic: 可以负数  [3. Strings, Vectors, and Arrays](# 3. Strings, Vectors, and Arrays)
...
When to use ```reinterpret_cast```: when interface with <span style="color:red">opaque data types(frequently occurs in vendor APIs over which programmer has no control). 下面例子是where a vendor provides an API for storing and retrieving arbitrary global data:
