...
- new standard lets a derived class explicitly note that function <span style="color:red"> **override** a virtual that it inherits</span> by <span style="color:red">specifying override after its **parameter list**, or **const**  keymember or **reference qualifier**</span>
...
- <span style="background-color:#FFFF00">**friendship is not transitive, friendship is not inherited**</span>. Transitive 表示 B 是 A 的友元, C是B的友元, 但是 C 不能访问A. **Inherited**: 可以通过friend 的 inherited class 访问 base class member(下面例子). The accesst to Base objects 是embedded in object derived from Base
...
- 如果想<span style="background-color:#FFFF00">**change access level**</span> of a name that derived class inherits. 可以这么做by providing a <span style="background-color:#FFFF00">**using declaration**</span> (using declaration <span style="color:red">**accessibility 取决于 access specifier**</span> )
    ...
    - using declaration is <span style="color:red">**protect**</span>, name is available to <span style="color:red">**memeber, friends, drived classes**</span>
...
- 每一个class 都define 了自己的scope. <span style="color:red">在nheritnace, scope of derived class nested inside scope of base classes</span>. 
...
- Name Collisions: a derived class can **reuse**(redefine) a name defined in one of its direct or indirect base classes. <span style="color:red">Names defined in an inner scope (e.g., a derived class) hide name in the outer scope(base class)</span>
    ...
    - 如果Derived class override function from base, 但也<span style="color:red">想要base all overloaded instances of that function available(而不是hide) in derived class cope</span>, 不用把base 每个function 重新写, 可以提供 <span style="color:red">**using declaration**</span>, specifies only a name, 不需要parameter list.
...
4. 假设call is legal, compiler generate code, 取决于if call is virtual or not
    ...
    - 如果 call is nonvirtual, or if the <span style="color:red">call is on object 而不是reference or pointer</span>, comiler gnerates a <span style="color:red">normal function call</span>, 即使是pointer 和 reference, 也是由static type 决定call 哪个version 
- 注意:  <span style="background-color:#FFFF00">**Name lookup before Type Checking**</span>: 如果a member in a derived class (i.e., in an inner scope) <span style="background-color:#FFFF00">**has the same name** as a base- class member</span>(i.e., a name defined in an outer scope), <span style="color:red">derived member 会hides base-class member 在derived class scope 内(即使function have different parameter lists is different for derived class and base class)</span>
...
- 接下来三条 calls are through pointers with differing types. ```bp2->f2(); ``` is illegal 因为没有 ```f2()``` in Base. 
...
- 通常来说如果有destructor, 也需要copy and asisgnment. 但是<span style="background-color:#FFFF00">有个重要例外</span>是: 如果base class 有empty virtual destructor, 无法推断base class 需要copy or assignment
  - base class needs a virtual destructor还有个重要影响:  <span style="background-color:#FFFF00">如果有destructor, even if use ```=default``` to use sythesize version, compiler 不会sythesize move operation for that class</span>
