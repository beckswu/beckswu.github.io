...
2. **Const Value-Value**: 可以用non const initialze ```const```  value ```int val = 5; const int cst = val;```, 同样也可以用 ```const```(reference or without reference)  initilize non const value; ```const int & cstRef = 3; int val = cstRef```
3. **Const Ref-Ref**: <span style="background-color"> const reference 不能bind 到non-const reference，因为reference is not object </span> ```const int & ref = 3; int &i = ref; \\error```, 反过来却可以 ```int val = 3; int & ref = val; const int &cstRef = ref;```
4. **Ref-Value**:
...
5. 生成```const reference```, 如果type 不匹配 or 用rvalue expression, 会生成一个temporary object, 这样如果改了原来的值, ```const reference```不会改 (因为```const reference```连得是temporary object)
...
上面例子中.To sure object to which ```ri``` is bound is an int, compiler transforms the code into something like (但不适用```ri2```, 因为initialize ```ri2```时, 没有生成temporary object)
