...
- copy constructor:  no 3 ,4 , 5, 6 defined 
- copy assignment operator: no 2, 4, 5, 6 defined 
- move constructor: no 2, 3, 4, 6 defined 
- move assignment operator: no 2, 3, 4, 5 defined 
...
- **rvalue-reference**: 只能绑定到 rvalue
   ...
   - <span style="background-color:#FFFF00">**r-value reference <-- rvalue reference**</span>, <span style="color:red">**Error**</span>: 因为rvalue reference variable 是lvalue (有identity)	
...
- **lvalue reference**: 要绑定到object 上
   ...
   -  <span style="background-color:#FFFF00">**lvalue <-- rvalue reference**✔️</span> 因为rvalue reference 表达式是 lvalue 
   - <span style="background-color:#FFFF00">**lvalue <-- lvalue**✔️</span>:
   - <span style="background-color:#FFFF00">**lvalue <-- lvalue reference**✔️</span>
...
int && rval_ref3 = rval_ref2; //error: cannot bind a rvalue reference  
...
- ```new``` . This function allocates <span style="color:red">**raw**, **untyped memory**</span> large enough to hold an object (or an array of objects) of the specified type. Then compiler runs appropriate constructor to <span style="color:red">construct the object(s)</span> from specified initializers. 最后 a <span style="color:red">**pointer to newly allocated and constructed object**</span> is returned
- ```delete```: <span style="color:red">the appropriate destructor is run</span> on the object to which ```sp``` points or on the elements in the array to which ```arr``` points. Next, <span style="color:red">the compiler **frees the memory**</span> by calling a library function named <span style="color:red">operator ```delete``` or operator ```delete[]```</span>, respectively.
- 可以定义自己的 ```new``` and ```delete```, we take over responsibility for all dynamic memory allocation. <span style="color:red">Functions 必须crorect</span>
    - 可以定义在global scope or as member functions 
    - 当compiler 发现 ```new``` or ```delete```, 如果object being allocated 是 class type, compiler 首先looks in class scope including any base classes. 如果发现, 就使用，否则, look for a matching function in global scope. 如果发现 user-defined version, it uses that function to execute the ```new``` or ```delete``` . 如果没有找到, use library version 
...
- <span style="background-color:#FFFF00">Library 定义了8个 overloaded versions of ```operator new``` and ```delete```. 前四个有可能throw ```bad_alloc``` exception. 后四个support nonthrowing versions of ```new``` </span>
    ...
    - 如果定义下面8个, 必须定义在<span style="color:red">**global scope**</span> or <span style="color:red">**class scope**</span>.
        ...
        - static 原因: ```new``` and ```delete``` functions 必须是 static 因为they are used either before object is constructede or after it has been destroyed. <span style="background-color:#FFFF00">因此```new``` 和 ```delete``` 不能manipulate member data</span>
- ```new```
