...
**this**: 
- ```this``` is a <span style="background-color:#FFFF00">const pointer</span>, we cannot change the address that ```this``` holds. It is a constant pointer that holds the memory address of current object. <span style="color:red">this is not available in static member function </span> 
- Member functions access the object through extra, implicit parameter named **this**. **this** is initialized with<span style="color:red"> the address of the object</span> which the function (只限于nonstatic) was invoked.
