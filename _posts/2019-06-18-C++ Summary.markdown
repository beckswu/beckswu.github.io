...
1. Level Const
   ...
   - **top level const**可以变成**low level const**,  e,g. ```const int a = 0; const int * aptr = &a``` ```a```表示top level const, 通过```&``` operator, 变成**low level const(aptr)**
2. <span style="background-color: #FFFF00">The distinction between **top-level** and **low-level** matters when we copy an object</span>. When we copy an object, **top-level consts can be ignored:** e.g. ```const * const int = const * int```, <span style="color:red">**Error**</span>, 
    -  <span style="background-color: #FFFF00">对于pointe之间r的copy, 如果忽略Low Level const</span>, <span style="color:red">**Error**</span> 
    ...
    - 对于value 到 const pointer 的copy, 可以忽略```low-level const```
