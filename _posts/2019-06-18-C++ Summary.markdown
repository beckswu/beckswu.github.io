...
- 如果 ```bp``` points to ```Derived``` object, cast will initialize ```dp``` to point to the ```Derived``` object to which ```bp``` points. <span style="background-color:#FFFF00">Safe io use ```Derived``` operations inside if </span>
