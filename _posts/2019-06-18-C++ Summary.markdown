...
2. **Stream iterator**: 这些迭代器被绑定到输入或者输出流上, 可以用来iteratre through the associated IO stream
...
- **inserter**: creates an iterator uses ```insert```. 此函数接受第二个参数, 这个参数必须是指向给定容器的iterator, 元素将被插入到给定iterator的前面, ```inserter(c,iter)```, 会插入到```iter```所指向元素的前面, ```c``` 是container
...
- ```istream_iterator``` use ```>>``` to read a stream. 
    ...
    - 还可以使用默认initialize iterator, which creates an iterator that we can use as the off-the-end value (可以当尾后使用的迭代器)
