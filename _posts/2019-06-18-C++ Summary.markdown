...
- accumulate 可以用 ```string("")``` 作为第三个参数, 但不能用 ```""``` 因为no accumlate for ```const char*```
...
- lambda capture by value: 捕获指 是 创建时 而不是调用时的值, 随后修改被调用没有影响
    - <span style="color:red">如果想改变 capture value 的值， 加mutable, 有mutable 不能省略 parameter list 可以省略return type </span>
...
- inherit constructor:
    ...
    - using declaration for constructor, generate code, ```using Base::Base``` 继承base all constructors
        ...
        - default arguments 会生成多个递减argument
