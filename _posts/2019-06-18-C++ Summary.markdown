...
- IO 类是不能被copy的类型,所以overload <<, >>, 带入ostream, istream reference
...
- <span style="background-color:#FFFF00">函数主体只能有一个return statement</span>(function body must contain exactly one return statement, 不能有通过if 判断的多个statement)
...
Functions defined in the classes are <span style="background-color:#FFFF00">implicitly inline</span>. 定义在class 内部的函数都是inline的. Although every member must be declared inside its class(必须在函数内部声明), we can define a member function’s body either inside or outside of the class body.
...
**this**: 
- Member functions access the object through extra, implicit parameter named **this**(通过this额外的隐式参数来访问调用的对象). 
- 当call member function, **this** is initialized with<span style="color:red"> the address of the object</span> which the function was invoked.
...
- ```this``` is a <span style="background-color:#FFFF00">const pointer</span>, we cannot change the address that ```this``` holds
- 在class 函数内部的直接access member of the object, <span style="background-color:#FFFF00">因为any direct use of a member of the class is assumed to be an implicit reference through ```this```</span>(任何对类成员直接访问都被看作this的隐式引用)
    - ```bookNo``` It is as if we write ```this->bookNo```

**const Member Functions**
- ```const``` is to modify the type of the implicit ```this``` pointer, ```const```keyword放在parameter list之后表示 ```this``` is a pointer to ```const```. member function that use ```const```被称为<span style="color:red">const member functions</span>
-  by default, the type of ```this``` is ```const``` pointer<span style="color:red"> to the non const version of the class type</span>(指针const, class 不是const).
- 尽管```this``` is implicit, 但是follow initialization rules, which means cannot bind ```this```（this 没有low level const） to a ```const``` object(比如 pointer to const value, , 因为const 通过function call, this is a const pointer to const object, 而non-const functio的this 是const pointer to nonconst obejct).  因为我们cannot call an ordinary member function on a const object; 
...
- ```this``` is a pointer to ```const``` 表示 <span style="background-color:#FFFF00">const member functions cannot change the object on which they are called</span>. 因此const function 不能write to data members of the objects 只能write
- <span style="background-color:#FFFF00">Objects that are const, and references or pointers to const objects, may call only const member functions.</span> (因为const pointer to const object, or const ref call 不是const function，有潜在改变object的风险, 所以不能call)
...
- Unlike other member functions, <span style="background-color:#FFFF00">constructor 不能被declared as const</span>. <span style="color:red">When we create a **const** object of a class type, the object 直到constructure completes the object's initialization 才获得constness 属性 </span>. constructors can write to const objects during their construction.
- Classes control default initialization by defining a special constructor, known as the **default constructor**.  <span style="background-color:#FFFF00">constructor 不能被declared as const. The default constructor is one that takes no arguments.</span> default constructor 是constructor work without any parameter; 错误的概念是constructor with no parameter; 比如```dog(string name = “Bob”); ```属于can work without any parameter
  - 如果我们explicitly define any constructors, <span style="color:red">compiler will implicitly define the default constructor</span>
  - <span style="background-color:#FFFF00">The compiler-generated constructor is known as the **synthesized default constructor**</span>, 对于大多数class, this synthesized constructor initializes each data member as folows: 
    - 如果there is an in-class initializer, 用它initialize member(如果存在class的初始值,用初始值初始化成员)
    - Otherwise, default-initialize the member(否则, 默认初始化成员)
  - ```Sales_data() = default;``` we need to provide default constructor as well as default constructor, <span style="color:red">The = default can appear with the declaration inside the class body or on the definition outside the class body(既可以和声明出现在class 内部, 也可以作为定义出现在class 外部)</span>. <span style="background-color:#FFFF00">如果用```=default``` inside class double, the default constructor will be inlined. 如果出现在definition outside the class, the member will not be inlined by default </span>
  - 如果compiler 支持in-class initializer (在class 内部```int a = 1;```) 我们可以用default constructor without parameters, 如果不支持, your default constructor should use the **constructor initailizer list** to initialize every member of the class (e.g. ```sale(): a(0) {}```, 函数体是空的, 因为目的是为了数据成员赋值, 一旦没有其他成员, 函数体就是空)
- constructor 不应该override in-class initializers except to use a different initial value.

**Some Classes Cannot Rely on the Synthesized Default Constructor**
- <span style="color:red">The compiler generates a default constructor automatically only if a class declares no constructors</span>. 只有我们没有定义任何的constructor，才会生成**synthesized default constructor**, 如果有自己定义constructor， 除非自己定义default constructor, 否则不会生成
- 第二原因是: synthesized default constructor does the wrong thing, 比如数组或者指针对象被默认初始化, 他们值是未定义的. 所以当class has compound type (pointer or array) or built-in 应该有自己定义的constructor, or user create objects with memebers that have undefined value 
...
- 通过定义data private, 作用可以自由修改数据,只要nterface 不变，用户代码不变，但是如果the data are public, then any code that used the old data members 也许brokern. 需要rewrite any code that relied on old representation 后才能使用
...
- <span style="color:red">如果Friend function定义在class 内部是inline function</span>
...
- <span style="background-color:#FFFF00">A const member function that returns *this as a reference should have a return type that is a reference to const.</span>一个const 成员函数如果return type 是 class reference 形式返回```*this```, 则return type 是 ```const class & ```
- <span style="background-color:#FFFF00">可以overload function 基于是不是const </span> (match rule 跟function overloading match rule 类似)
  - <span style="color:red">const object 只能call const function, 可能call nonconst function</span>
  - <span style="color:red">nonconst obeject 可以call non-const 也可以call const version， 但是nonconst version 是better match</span>
...
Returning *this from a const Member Function: 如果有一个新的function ```display``` return type 是 ```Screen &```, 但是<span style="color:red">const function</span>, 则下面是错误的. If ```display``` is a ```const``` member, then ```this``` is a pointer to ```const``` and ```*this``` is a ```const``` object. Hence, the return type of ```display``` must be const ```Screen&```. 即使```myScreen``` is non const, 原因是```display``` 的返回时const reference, we cannot call ```set ``` on a ```const``` object
```c++
Screen myScreen;
...
Const function overload, <span style="background-color:#FFFF00">object 是不是const, 决定call 哪个display function.</span>  <span style="color:red">```*this``` pointer is passed implicitly</span>. when ```display```calls ```do_display```, ```*this``` pointer is implicitly passed to ```do_display```. 当nonconst version of ```display``` calls ```do_display```, <span style="background-color:#FFFF00">```*this``` pointer is implicitly convert from a pointer to nonconst to a pointer to const </span>, 当```do_display```完成后，```display```函数各自返回解引用```this```所得的对象, nonconst version, ```*this``` points to a nonconst object, ```display``` 函数返回一个ordinary(普通的, non-const) reference, the const member returns a reference to const.
...
**Class Types**: 
- Every class deines a unique type, <span style="color:red">即使两个different types define the same members 他们也是不同类型的</span>. Even if two classes have exactly the same member list, they are different types. The members of each class are distinct from the members of any other class (or any other scope).
- 声明class object, 可以直接用class or struct name as type, 或者把name 放在keyword class or type的后面
- 也可以只声明class 而不定义class, like ```class Screen;```, 这种声明叫做**forward declaration**, 但在它声明之后 定义之前是一个**imcomplete type**, 因为不清楚它包含哪些成员
  - **imcomplete type** can bed used in limited ways: define pointers or references to such types, 也可以定义functions that use an imcomplete type as a parameter or return type 
- A class 必须被defined 而不仅仅是声明before we write code that creats objects of that type. 因为compiler 需要know how much storage such objects need. 同理, a class 必须被定义before a reference or pointer is used to access a member of the type. 如果不定义, compiler cannot know 它有什么member
- <span style="background-color:#FFFF00">A class member 可以有pointers or references of its own type. 因为class is considered declared (not yet defined) as soon as its class name has been seen</span>. 但是compiler 需要知道how much storage the data memeber requires, 因为class is not defined until its class body is complete, <span style="color:red">a class cannot have members ot is own type</span>
...
#### (d). Class Scope
- class 外部访问public data/function，只能通过object, reference, pointer; type members(typedef) from the class 可以访问 using the scope operator .
- 在class 外部定义function, 需要provide class name + function name. 当class name 被看见 知道function 属于class, function定义就可以随便用class member without using class name (因为everyname is seen in class scope)
- class 返回值声明在 class name 前, 因为any named in return type is outside scope 当定义function outside class. 因此<span style="color:red">return type需要specify class name, 如果返回类型是在class 内部定义的</span>
...
1. Look for declaration of the name in the block (before use of name) which the name was used. 只在名字现在块中(使用之前的)查找
2. 如果名字没有发现, look in the enclosing scope（继续查找外层作用域)
3. if no declaration found, program is error

class defintion are processed into: 
1. memeber declarations are compiled (编译成员声明)
...
- 因为函数体直到class 可见后 才processed. 所以function 可以用任何name defined inside the class. 假如function definition 和member declaration 同时进行, 比如order memeber functions 这样才能用已经被看见的名字
...
- <span style="color:red">tips:</span>: 定义type names 通常在class beginning, 这样any member that uses that type will be seen after the type name has been already been defined

看到```balance``` function, 寻找```Money```的声明,先看在Account class中是否声明, 没有. 然后需要declaration in enclosing scope(外层作用域). 找到typedef 对象return type; 另一方面, <span style="color:red">因为balance 函数的在class is seen之后才看见, 所以返回的bal 是class 的</span>, 而不是外层作用域的string
```c++
...
**(2).Type Names Are Special**: 一般来说, inner scope can redefine a name from an outer scope even if that name has already been used in the inner scope. 但是class 不行, <span style="color:red">if a member uses a name from an outer scope and that name is a type, then class 不能重新定义它</span> class 不能重新定义type, 如果这个type 被member function parameter/return 使用

```c++
typedef double Money;
class Account {
public:
    scope Money balance() { return bal; } 
...
1. 首先在member function内部寻找 在name 被使用之前)
2. 如果没有找到, 再到class 内部继续寻找declaration. All the members of the class are considered (class所有成员都被考虑)3. 如果没有找打, look for a declaration that is in scope before the member function defintiion. (在函数定义前的作用域, 因为函数可能定义在class外部(in file) ) 

当compiler 处理dummy_fcn 的乘法时， 首先寻找 ```height```  in the scope of that function. function parameter in function scope 所以使用的是parameter 的```height```
```c++
...
//即使height 出现在dummy_fcn 后面, 根据lookup rule会使用class member height (而不用外面的height)
...
注意 ```verify``` is not visible before the definition of the class screen. 但是第三步lookup includes the scope where member definition befores. 因为```verify```在```setHeight```前定义,因此可以使用 
```c++
int height; // defines a name subsequently used inside Screen 
...
Assignment 和 initialization 是不同的: <span style="background-color:#FFFF00">在class 中, Assignment是先初始化 再赋值， initializatin是直接初始化</span>, 比如下面例子, How significant the distinction between initialization and assignment 由data member 类型决定
...
- <span style="color:red">必须用**constructor initializer** list to privide values for members that are const, reference, or a class type that does have a default constructor</span>,const, reference 是必须initialized的
  - <span style="background-color:#FFFF00"> 如果member 有 const or reference, 不会有default constructor, 必须要Initialized </span>
  -  **constructor initializer list** specifies only the values used to initialize the members, not the order in which those initializations are performed. **constructor initializer list**没有说明初始化的order
-  <span style="background-color:#FFFF00">class members are initialized 与他们在class 出现的顺序一致, 因为 **constructor initializer list** order 不会影响他们实际初始化的顺序</span>
   -  一般顺序不matter, 但是如果一个成员用另一个成员初始化后的值, order 就matter了 (有些compiler 会有warning)
   -  最好write **constructor initializer** in the same order as members are declared. 其次避免using members to initialize other members
...
Order of Initialization, 下边例子中, **constructor initializer** 让 j is initialized then i. <span style="color:red">但实际上是i先initialized, 但是当initialized 时 with the undefined value of j</span>
```c++
class X{
    int i;
    int j;
...
**Default Arguments and Constructors**: the only difference between default constructor and takes a default parameter constructor: default constructor 用string的default constructor to initialize bookNo. constructor with a single argument 用argument to initialize ```bokNo```

<span style="background-color:#FFFF00">**A constructor that supplies default arguments for all its parameters also defines the default constructor**</span>
...
- In a delegating constructor, the member initializer list 有single entry call class another constructor. 
- When a constructor delegates to another constructor, 先run delegated-to constructor 之后才把control returned to the function body of delegating constructor(叫别的delegatted constructor的函数的主体)
...
- 当我们书写乳```T()```的表达式explicitly request value initialization where ```T``` is the name of a type (比如vector constructor takes a single argument to specify vector's size) 
