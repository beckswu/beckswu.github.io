...
- Implicit instantiation: 只有用到时候才实例化: declare pointer(只是declare没有绑定) to class, 不会instantiate classs. 或者当有derived-to-base pointer conversion时候 or  delete pointer
...
- ```template <typename T> void f(T &p);``` 如果有 const, 是lower-level, not top level. 
- ```template <typename T> void f(T &p);``` 只能pass lvalue, <span style="color:red">如果```T``` 含有const, 只能是lower-level</span>, 比如```const char * ```
...
- rvalue reference collasping
    ...
    - rvalue reference 只用于forwarding argument or template overloading
        ...
        - ```template <typename T> void f(const T&);```  lvalues and const rvalues, 因为有const conversion (不是top的)，所以上面的不是exact match
...
- ```forward<T>::type```
    ...
    - return rvalue reference (```T&&```) to that explicit argument type 
        - if argument is rvalue,  ```type``` is rvalue oridinary ty[e. return rvalue reference
...
- specialization 
    ...
    - declaration for a specialization must be in scope before any code uses that instantiation, 否则compiler generate code using template
- ```hash<key_type>```: 必须定义
    ...
    - 如果使用 key_type（```Sales_data```） 的 private type, 需要make ```hash<key_type>``` as ```Sales_data```friends
    - ```key_type``` 需要有 ```==``` 定义
...
e.g. <span style="background-color:#FFFF00">specialization 的 ```T``` 类型是 ```const char *```</span>, 我们template function 的parameter 是 ```const T&```, <span style="background-color:#FFFF00">这里const 表示是pointer 是const, 而不是a pointer to const</span>. 因此 need to use in specialization is ```const char* const &```, a reference to cosnt pointer to const char
