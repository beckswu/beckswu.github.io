...
**iteral type**: (int, int pointer, int reference, double, enum, char, char array char pointer) scalar type, reference type, an array of literal type;
...
**constexpr function**: 
- <span style="background-color:#FFFF00">return type must be literal type</span> (int, char)
