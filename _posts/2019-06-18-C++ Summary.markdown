...
- 尽管```this``` is implicit, 但是follow initialization rules, which means cannot bind ```this```（this 没有low level const） to a ```const``` object(比如 pointer to const value, , 因为const 通过function call, this is a cont pointer to const object, 而non-const functio的this 是const pointer to nonconst obejct).  因为我们cannot call an ordinary member function on a const object; 
