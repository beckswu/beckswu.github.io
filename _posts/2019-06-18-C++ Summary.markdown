...
- Pointer to class:
    ...
    - ```string Screen::*pdata;```: ```*```表示是 declaring a pointer
***
...
- linkage direcitive <span style="color:red">can be nested</span>, 比如一个header contains a function with its own linkage directive, likage of that function is unaffected.
...
- The language in which a function is written is part of its type. 因此 <span style="color:red">function declaration</span> with a linkage directive<span style="color:red"> 必须用一样的 linkage directive </span>
...
- 如果target language 支持overloaded function, it is likely that a compiler that implements linage directives for that language would support overloading of these function from C++. 
