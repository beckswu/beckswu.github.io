...
- one part of program detect problem and cass job of resolving to another part of program (Detecting part 不需要知道handling part)
...
- when ```throw``` executed, <span style="color:red">statement following ```throw``` 不被执行</span>. 而是 <span style="color:red">**control** is transferred from ```throw``` to matching ```catch```</span>. 
