...
$$dZ^{\left[ 2 \right]} = A^{\left[ 2 \right]} -  Y \text{  Y }, dZ^{\left[ 2 \right]}  \text{ is 1 by m matrix} $$<br/>
...
如果不加keepdims 可能产生 np.array funny (n, )array, 加上keepdims = true 给出$$\left( n^{\left[ 2 \right]} , 1\right)$$
