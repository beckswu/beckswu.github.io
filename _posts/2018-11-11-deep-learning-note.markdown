...
$$ dW^{\left[ 1 \right]} = \frac{1}{m}  \begin{bmatrix} ---z_1^{\left[ 1 \right]} --- \\  ---z_2^{\left[ 1 \right]}--- \\ \vdots \\ \underbrace{ ---z_n^{\left[ 1 \right]} ---}_{m 个columns} \end{bmatrix}    \begin{bmatrix}  \underbrace{-}_{\text{第一个attribute}}--x_1^{\left[ 1 \right]} --- \\  ---x_2^{\left[ 1 \right]}--- \\ \vdots \\ ---x_m^{\left[ 1 \right]} --- \end{bmatrix}      $$
