...
- Loss weights: 0s until the first ```<EOS>``` and then 1 on the start of the summary
  ...
  - Instead of averaging the loss for every word in the whole sequence, weight the loss for the words within the article with 0s and ones within the sumarry with 1s. So model only focus on summary
