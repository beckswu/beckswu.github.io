...
e.g. VB -> eat 0.5 means:  when model is currently at hidden state for a verb, 0.5 chance that observable the model will emit the word eats.  可以注意到每个word 的tag的概率都大于0，因为context 不同，tag不同, 例如 He lay on his *back* V.S. I'll be *back*
...
2. transform all wors in lowercase and keep punctuation 
...
Since vb is 1/3 for all outgoing transitions. Reasonable because don't have any data to estimate these probabilities
...
- <span style="background-color: #FFFF00">$$c_{k,j-1} $$: the probability of proceeding path traversed</span>
...
Extract the graph from matrix D, which represents most likely sequence of hidden states:  Get the index of entry of $$C_{i,k}$$ which is the highest probability in the last column C. Use that index s ($$\underset{i}{argmax} \ c_{i,k}$$) to traverse backwards through the matrix D to reconstruct the sequence 

e.g. 下面的例子,

1. 最后一列的最大的概率是 0.01 $$C_{1,5}$$, $$s = \underset{i}{argmax} \ c_{i,k} = 1$$. The most likely tag for word5 is tag1
...
$$C_\left(x \right) \quad \text{the count of all unigram x}$$
...
- Problem: first word, you don't previous word, can't calculate bigram probability
