...
$$ P\left(w_i \mid class \right) = \frac{freq\left(wi,class \right)}{N_{class}} \qquad \text{class}\in \text{{Positive, Negative}}$$
...
$$ P\left(w_i \mid class \right) = \frac{freq\left(wi,class \right) + 1}{N_{class} + V}$$
...
m gets larger, we can get numerical flow issues, hard to store on device , so we introduce the 

$$ log\left( \frac{P\left(pos\right)}{P\left(neg\right)} \prod_{i=1}^m \frac{P\left(w_i \mid pos\right)}{P\left(w_i \mid neg\right)} \right) =>  \underbrace{log\frac{P\left(pos\right)}{P\left(neg\right)}}_{\text{log prior}} + \underbrace{\sum_{i=1}^m \frac{P\left(w_i \mid pos\right)}{P\left(w_i \mid neg\right)}}_{\text{log likelihood}}  $$
 
 
log prior is important for unbalanced dataset. We further introduce λ as follows:

![](/img/post/Natural-Language-Processing/course1/week2pic7.png)

The positive value indicates tweet is positive. Negative value indicates tweet is negative
...
6. Compute $$logprior = log \frac{D_{pos}}{D_{neg}} $$ where $$ D_{pos} $$ and $$D_{neg}$$ coorespond to  the bumber of positive and negative documents respectively
