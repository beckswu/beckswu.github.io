...
- Matrix C: hold ther intermediate optimal probabilities 
  - first column C: the probability <span style="color:red">from start state $$\pi$$ to the first tag $$t_i$$</span> and  word $$W_1$$. $$  c_{i,1} = a_{1,i} \times b_{i,cindex\left(w_1\right)}$$: product of transition probability(from 1 to tag i) and emission probability(from tag i to word1)
