...
- Vector space: <span style="background-color: #FFFF00">identify the context around each word in the text to capture relative meaning, to represent words and documents as vector</span>
...
Problem with Euclidean distance:  Vector space where corpora are represented by occurrence of the words "disease" and "eggs". Argiculture and history corpus has a similar number of words. Suggests agriculture and history corpora are more similar than agriculture and food corpora

- If the angle is small, cosine -> 1, if angle is big, cosine -> 0
...
-  the cosine similarity values between 0 and 1
...
- Visualization to check if PCA capure relationship among words
- <span style="color:red">**Eigenvector**</span>: directions of uncorrelated features for your data
- <span style="color:red">**Eigenvalue**</span>: amount of information retained by each feature(variances of data in each of those new features)
- <span style="color:red">**Dot Product**</span>: gives the projection on uncorrelated features
