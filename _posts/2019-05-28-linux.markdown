...
注: 如果上面都是第一个command，都是<span style="background-color:#FFFF00">先把内容读到pattern</span>, 然后再```g```, ```G``` or ```H```
...
| pattern space before | hold space |  pattern space after  |
| :--- | :--- | :--- |
| one | ```""```   |  one  |
| two |   ```""```  | two <br/>```""```   |
| three | ```""```  | three <br/> ```""``` |
...
| pattern space before | hold space |  hold space after  |
| :--- | :--- | :--- |
| one | ```""```   |  one  |
| two <br/>```""```  |   ```""```  | two <br/> one  |
| three <br/> ```""``` | ```""```  | three <br/> two <br/> one |
